...
from app.schemas.user import UserInDBBase, UserCreate, UserWithRoles, UserBase
...
async def update_user(user_id: int, user: UserBase, auth: dict = Depends(get_session_identity), db: Session = Depends(get_db)):
    result = user_crud.update_user(db=db, user=user, user_id=user_id, tenant_id=auth["tenant_id"])
    if not result:
        raise HTTPException(status_code=404, detail="User not found")
